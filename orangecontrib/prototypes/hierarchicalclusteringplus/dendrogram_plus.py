...
from orangecontrib.prototypes.hierarchicalclusteringplus.hierarchical_plus \
    import Tree, postorder, preorder, leaves
...
def path_toQtPath(geom, width=1):
    p = QPainterPath()
    anchor, points = geom
    if len(points) > 1:
        if width > 1:
            if points[0][0]-points[1][0] > 0.05:
                x = points[0][0] - width*0.0001
            else:
                x = points[0][0]
            p.moveTo(x, points[0][1])
            for (x, y) in points[1:3]:
                p.lineTo(x, y)
            if  points[3][0]-points[2][0] > 0.05:
                x = points[3][0] - width*0.0001
            else:
                x = points[3][0]
            p.lineTo(x, points[3][1])
        else:
            p.moveTo(*points[0])
            for (x, y) in points[1:]:
                p.lineTo(x, y)
...
class DendrogramWidget(QGraphicsWidget):
    ...
    class ClusterGraphicsItem(QGraphicsPathItem):
        ...
        def boundingRect(self):
            ...
            if self.__boundingRect is None:
                sh = self.shape()
                pw = self.pen().widthF() / 2.0 # Returns the pen width with floating point precision.
    ...
    def setRoot(self, root):
        ...
        if root is not None:
            for node in postorder(root):
                pen = make_pen(Qt.blue, width=node.width, cosmetic=True,
    ...
    def _set_hover_item(self, item):
        ...
        if self._highlighted_item:
            for it in postorder(self._highlighted_item, branches):
                pen = make_pen(Qt.blue, width=it.node.width, cosmetic=True)
                it.setPen(pen)

        self._highlighted_item = item
        if item:
            for it in postorder(item, branches):
                hpen = make_pen(Qt.red, width=it.node.width,
                                cosmetic=True)
                it.setPen(hpen)
                if it.node.label:
                    self._highlighted_item.setToolTip(
                        self._nodeTooltip(it.node))

    def _nodeTooltip(self, node):
       return  "<table style='width:100%'> \
                  <tr><th>AU  </th><th>BP  </th></tr>\
                  <tr><td>"+ f"{node.label[0]}    " + "</td>\
                      <td>"+ f"{node.label[1]}    " + "</td></tr> </table>"
    ...
    def _relayout(self):
        ...
        for node_geom in postorder(self._layout):
            ...
            item.sourcePath = path_toQtPath(geom, node.width)
