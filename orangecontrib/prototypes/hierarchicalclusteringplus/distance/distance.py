...
class EuclideanRowsModel(FittedDistanceModel):
    ...
    def __init__(self, attributes, impute, normalize,
                 ...
                 dist_missing_disc, dist_missing2_disc, dist_metric, callback):
        super().__init__(attributes, dist_metric, 1, impute, callback,
                         normalize)
...
class EuclideanColumnsModel(FittedDistanceModel):
    ...
    def __init__(self, attributes, impute, normalize, means, stdvars,
                 dist_metric, callback=None):
        super().__init__(attributes, dist_metric, 0, impute, callback,
                         normalize)
        self.means = means
        self.vars = stdvars
...
class Euclidean(FittedDistance):
    ...
    def __new__(cls, e1=None, e2=None, axis=1, impute=False, normalize=False,
                callback=None, dist_metric=None):
        # pylint: disable=arguments-differ
        return super().__new__(cls, e1, e2, axis, impute, callback,
                               dist_metric=dist_metric, normalize=normalize)
    ...
    def fit_cols(self, attributes, x, n_vals):
        ...
        return EuclideanColumnsModel(
            ...
            self.dist_metric, self.callback)
...
class ManhattanRowsModel(FittedDistanceModel):
    ...
    used for scaling.
    """
    def __init__(self, attributes, impute, normalize, continuous, discrete,
                 medians, mads, dist_missing2_cont, dist_missing_disc,
                 dist_missing2_disc, dist_metric, callback=None):
        super().__init__(attributes, dist_metric, 1, impute, callback,
                         normalize)
        self.continuous = continuous
        self.discrete = discrete
...
class ManhattanColumnsModel(FittedDistanceModel):
    ...
    def __init__(self, attributes, impute, normalize, medians, mads,
                 dist_metric, callback=None):
        super().__init__(attributes, dist_metric, 0, impute, callback, normalize)
        self.medians = medians
        self.mads = mads
...
class Manhattan(FittedDistance):
    ...
    def __new__(cls, e1=None, e2=None, axis=1, impute=False, normalize=False,
        ...
        # pylint: disable=arguments-differ
        return super().__new__(cls, e1, e2, axis, impute, callback,
                               dist_metric, normalize=normalize)
...
class Cosine(FittedDistance):
    ...
    def fit_rows(self, attributes, x, n_vals):
        ...
        return self.CosineModel(attributes, self.axis, self.impute, discrete,
                                means, self.callback, self.dist_metric)
    ...
    class CosineModel(FittedDistanceModel):
        ...
        def __init__(self, attributes, axis, impute, discrete, means,
                     callback, dist_metric):
            super().__init__(attributes, dist_metric, axis, impute,
                             callback)
            self.discrete = discrete
            self.means = means
...
class JaccardModel(FittedDistanceModel):
    ...
    """
    def __init__(self, attributes, axis, impute, ps, callback, dist_metric):
        super().__init__(attributes, dist_metric, axis, impute, callback)
        self.ps = ps
...
class Jaccard(FittedDistance):
    ...
    def fit_rows(self, attributes, x, n_vals):
        ...
        return JaccardModel(attributes, self.axis, self.impute,
                            ps, self.callback, self.dist_metric)
...
class CorrelationDistanceModel(DistanceModel):
    ...
    def __init__(self, absolute, dist_metric, axis=1, impute=False):
        super().__init__(axis, dist_metric, impute)
...
class SpearmanR(CorrelationDistance):
    def fit(self, _):
        return SpearmanModel(False, self.dist_metric, self.axis, self.impute)


class SpearmanRAbsolute(CorrelationDistance):
    def fit(self, _):
        return SpearmanModel(True, self.dist_metric, self.axis, self.impute)
...
class PearsonR(CorrelationDistance):
    def fit(self, _):
        return PearsonModel(False, self.dist_metric, self.axis, self.impute)


class PearsonRAbsolute(CorrelationDistance):
    def fit(self, _):
        return PearsonModel(True, self.dist_metric, self.axis, self.impute)
...
class Bhattacharyya(Distance):
    ...
    def fit(self, data):
        return BhattacharyyaModel(self.axis, self.dist_metric, self.impute)
...
class Mahalanobis(Distance):
    ...
    def fit(self, data):
        ...
        return MahalanobisModel(self.axis, self.impute, vi, self.dist_metric)


class MahalanobisModel(DistanceModel):
    def __init__(self, axis, impute, vi, dist_metric):
        super().__init__(axis, dist_metric, impute)
        self.vi = vi
...
class Hamming(Distance):
    ...
    def fit(self, data):
        if self.axis == 0:
            return HammingColumnsModel(self.axis, self.dist_metric,
                                       self.impute)
        else:
            return HammingRowsModel(self.axis, self.dist_metric,
                                    self.impute)
...
class HammingColumnsModel(DistanceModel):
    ...
    """
    def __call__(self, e1, e2=None, impute=None,  dist_metric=None):
        if impute is not None:
            self.impute = impute
        return super().__call__(e1, e2)
