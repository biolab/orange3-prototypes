...
from orangecontrib.prototypes.hierarchicalclusteringplus.distmatrix import DistMatrix
...
class Distance:
    ...
    will execute the above steps and return :obj:`~rangecontrib.prototypes.hierarchicalclusteringplus.distmatrix.DistMatrix`.
    ...
    def __new__(cls, e1=None, e2=None, axis=1, impute=False,
                callback=None, dist_metric=None, normalize=False, **kwargs):
...
class DistanceModel:
    ...
    """
    def __init__(self, axis, dist_metric, impute=False, callback=None,
                 normalize=False):
        self._axis = axis
        self.impute = impute
        self.callback = callback
    ...
    def __call__(self, e1, e2=None):
        ...
        Returns:
            A distance matrix (
            orangecontrib.prototypes.hierarchicalclusteringplus.distmatrix.DistMatrix)
        """
        ...
        with np.errstate(invalid="ignore"):  # nans are handled below
            ...
            if isinstance(e1, (Table, RowInstance)):
                dist = DistMatrix(dist, self.dist_metric, e1, e2, self.axis,
...
class FittedDistanceModel(DistanceModel):
    ...
    """
    def __init__(self, attributes, dist_metric, axis=1, impute=False,
                 callback=None, normalize=False):
        super().__init__(axis, dist_metric, impute, callback, normalize)
...
class SklDistance:
    ...
    def __call__(self, e1, e2=None, axis=1, impute=False):
        ...
        if impute and np.isnan(dist).any():
            dist = np.nan_to_num(dist)
        if isinstance(e1, (Table, RowInstance)):
            dist_matrix = DistMatrix(dist, self.metric, e1, e2, axis,
                                     normalize=self.normalize)
        else:
