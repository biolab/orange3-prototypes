...
class DistMatrix(np.ndarray):
    ...
    """
    def __new__(cls, data, dist_metric, row_items=None, col_items=None,
                axis=1, normalize=False):
    ...
    def __array_finalize__(self, obj):
        ...
        if obj is None:
            return
        self.normalize = getattr(obj, 'normalize', False)
        self.dist_metric = getattr(obj, 'dist_metric', None)
    ...
    def __reduce__(self):
        ...
        newstate = state[4] + (self.dist_metric, self.row_items,
                               self.col_items, self.axis, self.normalize)
        return state[0], state[1], state[2], state[3], newstate

    # noinspection PyMethodOverriding,PyArgumentList
    def __setstate__(self, state):
        self.normalize = state[-5]
        self.dist_metric = state[-4]
    ...
    def from_file(cls, filename):
        ...
        return cls(matrix, dist_metric, row_labels, col_labels, axis,
                   normalize)
    ...
    def save(self, filename):
        ...
        :obj:`~orangecontrib.prototypes.hierarchicalclusteringplus
        .distmatrix.DistMatrix.from_file`.
        ...
        if not symmetric:
            data += "\tasymmetric"
        data += f"\tdist_metric={self.dist_metric}"
        data += f"\tnormalize={self.normalize}"
