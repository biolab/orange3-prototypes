...
from orangecontrib.prototypes.hierarchicalclusteringplus.distance import \
    Euclidean, PearsonR
...
class Tree(object):
    __slots__ = ("__value", "__branches","__width", "__label", "__hash")

    def __init__(self, value, branches=(), width=(1), label=()):
        ...
        self.__hash = hash((value, branches, width, label))
    ...
    def __iter__(self):
        return iter((self.__value, self.__branches, self.__width,
                     self.__label))

    def __repr__(self):
        return ("{0.__name__}(value={1!r}, branches={2!r}, width={"
                "3!r}, label={4!r})".format(
            type(self), self.value, self.branches, self.width, self.label))
    ...
    width = property(attrgetter("_Tree__width"))
    label = property(attrgetter("_Tree__label"))
...
def _update_width_and_label(significance, threshold, n_clusters, scale):
    widths = {i: 1 for i in range(0, n_clusters)}
    labels = {i: () for i in range(0, n_clusters)}
    if significance:
        dict_widths = {}
        min_width = 2 * scale
        for i in range(threshold, 101):
            dict_widths[i] = round(min_width, 1)
            min_width += 0.15

        for k, (value1, value2) in significance.items():
            value1, value2 = int(value1*100), int(value2*100)
            labels[k] = (value1, value2)
            widths[k] = dict_widths[value1] if value1 >= threshold  and \
                                               value1 <= 100 else 1
    return widths, labels


def tree_from_linkage(linkage, significance=None, threshold=95, scale=1):
    """
    ...
    for node in postorder(root):
        if node.is_leaf:
            ...
            t = Tree(
                ...
                tuple(T[ch] for ch in node.branches), node.width, node.label
            )
...
def prune(cluster, level=None, height=None, condition=None, significance=None):
    """
    ...
    """
    if not any(arg is not None for arg in [level, height, condition, significance]):
        raise ValueError("At least one pruning argument must be supplied")

    level_check = height_check = condition_check = significance_check = \
        lambda cl: False
    ...
    if significance is not None:
        cluster_sig = cluster_significance(cluster)
        significance_check = lambda cl: cluster_sig[cl] < significance


    def check_all(cl):
        return level_check(cl) or height_check(cl) or condition_check(cl) or\
               significance_check(cl)
    ...
    for node in postorder(cluster):
        if check_all(node):
            if node.is_leaf:
                ...
                if significance:
                    c = _check_children(node, significance, True)
                    if c:
                        T[node] = Tree(node.value, (), node.width, node.label)
                    else:
            ...
            T[node] = Tree(node.value,
                           tuple(T[ch] for ch in node.branches), node.width,
                           node.label)
    return T[cluster]


def _check_children(node, threshold, parent):
    for n in preorder(node):
        if n.left.label[0] < threshold and n.right.label[0] < threshold:
            left = _check_child(n.left, threshold, parent)
            right = _check_child(n.right, threshold, parent)
            if left and right and parent:
                return True
        return False


def _check_child(node, threshold, parent):
    if node.is_leaf:
        if parent:
            return True
        else:
            return False
    p = False if node.label[0] > threshold else True
    if node.left.label[0] < threshold and node.right.label[0] < threshold:
        left = _check_child(node.left, threshold, p)
        right = _check_child(node.right, threshold, p)
        if left and right:
            return True
    else:
        False
...
def cluster_significance(cluster):
    """
    Return a dictionary mapping :class:`Tree` instances to their
    significance value.
    ...
    """
    significance = {}
    for node in preorder(cluster):
        labels = node.label
        significance[node] = labels[0]
    return significance
