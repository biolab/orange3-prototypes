...
class OWImageGrid(widget.OWWidget):
    ...
    def process(self, size_x=0, size_y=0):
        if self.sim_grid:
            self.sim_grid.process(size_x, size_y)
    ...
    def setupScene(self, process_grid=True):
        ...
        if self.data:
            ...
            if process_grid and self.sim_grid:
                self.process()
                self.columns = self.sim_grid.size_x
                self.rows = self.sim_grid.size_y

            self.thumbnailView.setFixedColumnCount(self.columns)
            self.thumbnailView.setFixedRowCount(self.rows)

            for i, inst in enumerate(self.sim_grid.image_list):
    ...
    def commit(self):
        if self.data:
            if self.selectedIndices:
                ...
                selected = self.sim_grid.image_list[indices]
            else:
                selected = None
