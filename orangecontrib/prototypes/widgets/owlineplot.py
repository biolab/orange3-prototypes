...
class LinePlotViewBox(ViewBox):
    ...
    def add_profiles(self, y):
        ...
        if sp.issparse(y):
            y = y.todense()
        table = np.array([np.vstack((x[:, i], y[:, i].flatten())).T
                          for i in range(n)])
        self._profile_items = table
        ...
        if self._graph_state == SELECT and axis is None and self._can_select:
            ...
            if event.button() == Qt.LeftButton:
                ...
                if event.isFinish():
                    ...
                    self.selection_changed.emit(self.select_by_line(p1, p2))
...
class ProfileGroup:
    def __init__(self, data, indices, color, graph):
        self.n_vertex = data.X.shape[1]
        self.x_data = np.arange(1, self.n_vertex + 1)
        self.y_data = data.X
    ...
    def __get_disconnected_curve_data(self, y_data):
        x = np.arange(y_data.shape[0] * self.n_vertex) % self.n_vertex + 1
        y = y_data.A.flatten() if sp.issparse(y_data) else y_data.flatten()
        connect = np.ones_like(y, bool)
        connect[self.n_vertex - 1:: self.n_vertex] = False
        return x, y, connect
    ...
    def update_profiles_color(self, selection):
        ...
        alpha = LinePlotStyle.UNSELECTED_LINE_ALPHA if not selection \
            else LinePlotStyle.UNSELECTED_LINE_ALPHA_WITH_SELECTION
