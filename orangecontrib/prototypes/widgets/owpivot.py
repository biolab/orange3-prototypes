...
from AnyQt.QtCore import Qt, QItemSelection, QItemSelectionModel, pyqtSignal
...
class Pivot:
    ...
    def __group_aggregations(self, agg_funs):
        ...
        var_dep_funs = []
        prod = product(self._table.domain.variables + self._table.domain.metas,
    ...
    def _create_group_tables(self, var_indep_funs, var_dep_funs):
        attrs = [ContinuousVariable(f"{fun}") for fun in var_indep_funs]
        for var, fun in var_dep_funs:
            if fun in self.DiscVarFunctions:
                attrs.append(DiscreteVariable(f"{var.name}_{fun}", var.values))
            else:
                attrs.append(ContinuousVariable(f"{var.name}_{fun}"))
        ...
        for i, comb in enumerate(combs):
            ...
            for j, fun in enumerate(var_indep_funs):
                if fun in self._indepen_agg_done:
                    ...
                    X[i, j] = fun(sub_table)
            for k, (var, fun) in enumerate(var_dep_funs, j + 1):
                if fun in self._depen_agg_done:
                    X[:, k] = group_tab.X[:, self._depen_agg_done[fun][var] - offset]
                else:
                    X[i, k] = fun(sub_table.get_column_view(var)[0])
    ...
    def __get_pivot_tab_domain(self, val_var, X, X_h, X_v, X_t, agg_funs):
        ...
        if not val_var or val_var.is_continuous:
            ...
            attrs = [[cv(f"{v}", 1) for v in self._col_var.values]] * 2
            attrs.extend([[cv("Total", 1)]] * 2)
        else:
            attrs = []
            for x in (X, X_h):
                attrs.append([DiscreteVariable(f"{v}", map_values(i, x))
                              for i, v in enumerate(self._col_var.values, 2)])
            for x in (X_v, X_t):
    ...
    Mean.func = lambda x: nanmean(x[0], axis=0) if x[0].shape[0] > 0 else np.nan
    Min.func = lambda x: nanmin(x[0], axis=0) if x[0].shape[0] > 0 else np.nan
    Max.func = lambda x: nanmax(x[0], axis=0) if x[0].shape[0] > 0 else np.nan
    Median.func = lambda x: nanmedian(x[0], axis=0) if x[0].shape[0] > 0 else np.nan
    Mode.func = lambda x: nanmode(x[0], axis=0).mode if x[0].shape[0] > 0 else np.nan
    Var.func = lambda x: nanvar(x[0], axis=0) if x[0].shape[0] > 0 else np.nan
...
class PivotTableView(QTableView):
    ...
    def __cell_clicked(self, model_index):
        ...
        elif j == self._n_leading_cols - 1 or j == n - 1 or j == 1:
...
class OWPivot(OWWidget):
    ...
    def _add_main_area_controls(self):
        ...
        gui.rubber(box)
        gui.comboBox(box, self, "val_feature", label="Show: ",
