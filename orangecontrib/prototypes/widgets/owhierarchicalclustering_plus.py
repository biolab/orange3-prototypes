...
from orangecontrib.prototypes.hierarchicalclusteringplus.hierarchical_plus \
    import leaves, postorder, top_clusters, preorder, prune, \
    dist_matrix_linkage, tree_from_linkage
from orangecontrib.prototypes.hierarchicalclusteringplus.dendrogram_plus \
    import DendrogramWidget
from  orangecontrib.prototypes.hierarchicalclusteringplus.distmatrix import \
    DistMatrix
from orangecontrib.prototypes.hierarchicalclusteringplus.pvclust import PvClust

__all__ = ["OWHierarchicalClusteringPlus"]
...
class SignificanceRunner:
    @staticmethod
    def run(data: Orange.data.Table, method: LINKAGE, metric,
            nboot: int, normalize: bool, state: TaskState):
        ...
        kwargs = { "method": method, "metric": metric, "nboot": nboot,
                   "normalize": normalize, "callback": callback}
        res = PvClust(data, **kwargs)
        return  res.result


class OWHierarchicalClusteringPlus(widget.OWWidget, ConcurrentWidgetMixin):
    name = "Hierarchical Clustering Plus"
    ...
    significance = settings.Setting(0)
    #: Significance threshold
    threshold = settings.Setting(2)
    #: Number of bootstrap samples
    samples = settings.Setting(0)
    #: Branch thickness
    thickness = settings.Setting(1)
    ...
    bootstrap_samples = [100, 200, 500, 1000]

    significance_threshold = [80, 90, 95, 99]
    ...
    def __init__(self):
        ...
        self._threshold_prune_button = gui.appendRadioButton(
            box, "Threshold:", addToLayout=False)
        self._threshold_prune_button.setEnabled(False)
        grid.addWidget(self._threshold_prune_button, 2, 0)

        self._threshold_prune_values = gui.comboBox(
            box, self, 'threshold', items=self.significance_threshold,
            addToLayout=False, callback=self._threshold_prune_changed,
            label=None)
        self._threshold_prune_values.setEnabled(False)
        grid.addWidget(self._threshold_prune_values, 2, 1)
        ...
        self._threshold_selection_button = gui.appendRadioButton(
            self.selection_box, "Threshold:", addToLayout=False)
        self._threshold_selection_button.setEnabled(False)
        grid.addWidget(self._threshold_selection_button, 3, 0)

        self._threshold_selection_values = gui.comboBox(
            self.selection_box, self, "threshold",
            items=self.significance_threshold, addToLayout=False,
            callback=self._threshold_selection_changed, label=None)
        self._threshold_selection_values.setEnabled(False)
        grid.addWidget(self._threshold_selection_values, 3, 1)
        ...
        significance_box = gui.widgetBox(self.controlArea, "Significance")
        grid = QGridLayout()
        significance_box.layout().addLayout(grid)

        self._significance_check_box = gui.checkBox(
            significance_box, self, "significance", "Threshold:",
            callback=self._significance_changed
        )
        grid.addWidget(self._significance_check_box, 0, 0)

        self._significance_threshold = gui.comboBox(
            significance_box, self, "threshold",
            items=self.significance_threshold, callback=self._update_thickness)
        grid.addWidget(self._significance_threshold, 0, 1)

        grid.addWidget(
            gui.label(significance_box, self, label="Boostrap samples:"), 1, 0)
        self._bootstrap_samples = gui.comboBox(
            significance_box, self, "samples", items=self.bootstrap_samples,
            callback=self._significance_changed)
        grid.addWidget(self._bootstrap_samples, 1, 1)

        grid.addWidget(
            gui.label(significance_box, self, label="Branch thickness:"))
        self._thickness_slider = gui.hSlider(
            significance_box, self, "thickness", minValue=2, maxValue=4,
            step=1, ticks=True, createLabel=False,
            callback=self._update_thickness)
    ...
    def _update(self):
        ...
        if distances is not None:
            ...
            tree = tree_from_linkage(Z, self._significance, self._threshold,
                                     self._thickness_scale)
            self.linkmatrix = Z
            self.root = tree
            ...
            if self.pruning == 1:
                ...
                threshold = self._threshold_prune_values.currentText()
                self._set_displayed_root(
                    prune(self.root, significance=int(threshold)))
            else:
    ...
    def _clear_significance(self):
        ...
        self._significance = None
        self._significance_check_box.setChecked(False)
    ...
    def select_threshold(self, threshold):
        root = self._displayed_root
        if root:
            clusters = significant_clusters(root, threshold)
            self.dendrogram.set_selected_clusters(clusters)
    ...
    def _update_thickness(self):
        self._threshold = int(self._significance_threshold.currentText())
        self._thickness_scale = self._thickness_slider.value()/2
        self._update()

    def _threshold_prune_changed(self):
        self._update_thickness()
        self._invalidate_pruning()

    def _threshold_selection_changed(self):
        self._update_thickness()
        self._selection_method_changed()

    def _significance_changed(self):
        if self._significance_check_box.isChecked():
            method = LINKAGE[self.linkage].lower()
            metric = self.matrix.dist_metric
            data = self.matrix.row_items if self.matrix.axis == 0 else \
                self.matrix.row_items.transpose(self.matrix.row_items)
            nboot = int(self._bootstrap_samples.currentText())
            normalize = self.matrix.normalize
            self.start(SignificanceRunner.run, data, method, metric, nboot,
                       normalize)
        else:
            self.pruning = 0 if self.pruning==2 else self.pruning
            self.selection_method = 0 if self.selection_method==3 else \
    ...
    def on_done(self, result):
        self._significance = result
        self._threshold_prune_button.setEnabled(True)
        self._threshold_prune_values.setEnabled(True)
        self._threshold_selection_button.setEnabled(True)
        self._threshold_selection_values.setEnabled(True)
        self._update()
    ...
    def _apply_selection(self):
        ...
        if self.selection_method == 0:
            ...
            self.select_threshold(self._threshold)
...
def significant_clusters(node, threshold):
    left = _check(node.left, threshold, [])
    right = _check(node.right, threshold, [])
    clusters = list(set(left + right))
    return clusters


def _check(node, threshold, clusters):
    if node.is_leaf:
        return clusters
    if node.label[0] < threshold:
        _check(node.left, threshold, clusters)
        _check(node.right,threshold, clusters)
    else:
...
if __name__ == "__main__":  # pragma: no cover
    from orangecontrib.prototypes.hierarchicalclusteringplus import distance
    import Orange
    data = Orange.data.Table("housing")
    matrix = distance.Euclidean( distance._preprocess(data), axis=0,
                                 normalize=True,
                                 dist_metric=distance.distance.Euclidean)
    WidgetPreview(OWHierarchicalClusteringPlus).run(matrix)
